...
# Gruppo Staff Admin per notifiche pagamenti
...
def get_main_keyboard(user_status: str, user_id: int = None) -> InlineKeyboardMarkup:
    ...
    if user_status == 'subscribed':
        ...
        # Se l'utente è admin, aggiungi sezione admin
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Supporto
        ...
        keyboard = InlineKeyboardMarkup([
            # SUPPORTO INTERNO (Pagamenti/Abbonamento)
            ...
            # REDIRECT AL BOT SUPPORTO TECNICO
            ...
            [InlineKeyboardButton("📚 Contenuti/Risorse", url=SUPPORT_BOT_LINK)],
            [InlineKeyboardButton("❓ Altro", url=SUPPORT_BOT_LINK)],
            [InlineKeyboardButton("🔙 Indietro", callback_data='back_to_menu')]
        ])
        
        await query.edit_message_text(
            "🎫 *SUPPORTO*\n\n"
            "*💳 Problemi di Pagamento/Abbonamento:*\n"
            "Seleziona una delle opzioni qui sotto.\n\n"
            "*🔧 Supporto Tecnico:*\n"
            f"Usa il bot dedicato {SUPPORT_BOT_USERNAME}\n"
            "per assistenza tecnica rapida!",
            parse_mode='Markdown',
            reply_markup=keyboard
        )
    
    # Separatore supporto (non fa nulla)
    elif data == 'support_separator':
        await query.answer()
    
    # SUPPORTO INTERNO: Pagamento
    elif data == 'support_payment':
        context.user_data['support_category'] = 'payment'
        context.user_data['support_type'] = 'internal'
        
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("❌ Annulla", callback_data='support')]
        ])
        
        await query.edit_message_text(
            "💳 *PROBLEMI DI PAGAMENTO*\n\n"
            "Descrivi il tuo problema con il pagamento:\n\n"
            "• Quale errore vedi?\n"
            "• Quale carta/metodo stai usando?\n"
            "• Quando è successo?\n\n"
    ...
    # SUPPORTO INTERNO: Abbonamento
        ...
        await query.edit_message_text(
            ...
            "Descrivi il tuo problema con l'abbonamento:\n\n"
...
async def support_description_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Riceve la descrizione del problema di supporto (INTERNO - solo pagamenti/abbonamento)."""
    user = update.effective_user
    ...
    category_names = {
        'payment': '💳 Pagamenti',
        'subscription': '⚙️ Abbonamento',
    }
    
    category_emoji = '💳' if category == 'payment' else '⚙️'
    
    await update.message.reply_text(
        f"✅ *Ticket #{ticket_id} Creato!*\n\n"
        f"📌 Categoria: {category_names.get(category, category)}\n"
        f"⏰ Tempo stimato: 2-4 ore\n\n"
        "Il team amministrativo è stato notificato.\n"
    ...
    # === NOTIFICA AL GRUPPO STAFF ADMIN ===
    try:
        staff_keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("📞 Contatta Utente", url=f'tg://user?id={user.id}')]
        ])
        
        await context.bot.send_message(
            chat_id=STAFF_ADMIN_GROUP_ID,
            text=(
                f"{category_emoji} *NUOVO TICKET #{ticket_id}*\n\n"
                f"👤 *Utente:* @{user.username or 'N/A'}\n"
                f"🆔 *ID:* `{user.id}`\n"
                f"👤 *Nome:* {user.first_name} {user.last_name or ''}\n"
                f"📁 *Categoria:* {category_names.get(category, category)}\n"
        ...
        logger.error(f"Errore notifica staff admin: {e}")
...
async def addadmin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_super_admin(user.id):
        await update.message.reply_text("❌ Solo il Super Admin può aggiungere admin.")
        return
    ...
    if not context.args:
        await update.message.reply_text(
            ...
            "Per ottenere l'user_id, l'utente può scrivere a @userinfobot",
            parse_mode='Markdown'
        )
        return
    ...
    if add_admin(target_user_id, user.id, username, first_name):
        ...
        await update.message.reply_text("ℹ️ L'utente è già admin.")
...
async def removeadmin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if remove_admin(target_user_id, user.id):
        ...
        await update.message.reply_text("❌ Utente non trovato tra gli admin.")
...
async def listadmin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_super_admin(user.id):
        await update.message.reply_text("❌ Solo il Super Admin può vedere la lista admin.")
        return
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === GESTIONE TICKET PAGAMENTI/ABBONAMENTO ===
    elif query.data.startswith('ticket_take_'):
        user = update.effective_user
        ticket_id = int(query.data.replace('ticket_take_', ''))
        
        # Aggiorna messaggio con chi ha preso in carico
        await query.edit_message_text(
            query.message.text + f"\n\n✅ *Preso in carico da @{user.username or user.first_name}*",
            parse_mode='Markdown'
        )
        
        log_activity(user.id, 'ticket_take', f'Ticket #{ticket_id} preso in carico')
    
    elif query.data.startswith('ticket_close_'):
        user = update.effective_user
        ticket_id = int(query.data.replace('ticket_close_', ''))
        
        # Chiudi il ticket nel database
        try:
        ...
        # Aggiorna messaggio
        await query.edit_message_text(
            query.message.text + f"\n\n✅ *RISOLTO da @{user.username or user.first_name}*",
            parse_mode='Markdown'
        )
        
        log_activity(user.id, 'ticket_close', f'Ticket #{ticket_id} chiuso')
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(admin_callback, pattern='^ticket_'))  # Gestione ticket
