...
def create_checkout_session(user_id: int, user_email: str = None) -> str:
    ...
    logger.info(f"STRIPE_SECRET_KEY inizia con: {STRIPE_SECRET_KEY[:10] if STRIPE_SECRET_KEY else 'NONE'}...")
    ...
    try:
        ...
        logger.error(f"=== ERRORE STRIPE DETTAGLIATO ===")
        logger.error(f"Tipo errore: {type(e).__name__}")
        logger.error(f"Messaggio: {e}")
        logger.error(f"User message: {e.user_message if hasattr(e, 'user_message') else 'N/A'}")
        logger.error(f"=================================")
        raise
